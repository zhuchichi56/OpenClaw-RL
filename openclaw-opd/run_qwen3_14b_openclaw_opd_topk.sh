#!/bin/bash
# OpenClaw OPD — Qwen3-14B Full-Parameter (No LoRA)
#
# GPU layout: 4x H100 80GB
#   actor=2 (TP=2), rollout=1 (TP=1), PRM=1 (TP=1)
#   14B bf16 ≈ 28GB per full copy; fits in single H100 for inference
#
# Key differences from 1.7B:
#   - Model: Qwen3-14B (40 layers, hidden=5120, ffn=17408, 40 heads, 8 KV groups)
#   - Lower LR: 5e-6 (larger model needs smaller LR)
#   - Lower max-tokens-per-gpu: 2048 (14B needs more memory per token)
#   - TP=1 for rollout/PRM SGLang (single GPU can hold 14B)
#   - Reduced sglang-mem-fraction-static: 0.75 (leave room for KV cache)
...
# Source model architecture args for Megatron backend (14B)
source "${SLIME_ROOT}/scripts/models/qwen3-14B.sh"

HF_CKPT=${HF_CKPT:-${REPO_ROOT}/models/Qwen3-14B}
REF_LOAD=${REF_LOAD:-${HF_CKPT}}
SAVE_CKPT=${SAVE_CKPT:-/workspace/OpenClaw-RL/saves/openclaw/qwen3-14b-opd}
...
export OPENCLAW_RECORD_FILE="${SCRIPT_DIR}/results/qwen3_14b_opd_topk_record.jsonl"
export TP="${TP:-1}"
